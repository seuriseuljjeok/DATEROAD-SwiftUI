...
enum LocationFilter {
    ...
    enum City: Identifiable {
        ...
        enum Gyeonggi: String, CaseIterable, Identifiable {
        ...
        enum Incheon: String, CaseIterable, Identifiable {
...
struct LocationFilterCityKorToEng {
...
extension LocationFilter.City: CaseIterable {
    
    static var allCases: [LocationFilter.City] {
        return [
            // 서울 관련 도시
            .seoul(.all), 
...
struct LocationMapper {
    ...
    static func getCountryAndCity(from cityName: String) -> (country: LocationFilter.Country, city: LocationFilter.City)? {
        ...
        if let city = LocationFilter.City.allCases.first(where: { $0.rawValue == cityName }) {
            switch city {
            case .seoul:
...
struct LocationFilterCityEngToKor {
